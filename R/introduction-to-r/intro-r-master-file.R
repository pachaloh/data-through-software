...
# 
# chisq.test(table(wra.data$anemia, wra.data$mtype))
# chisq.test(table(wra.data$anemia, wra.data$mregion))
# chisq.test(table(wra.data$anemia, wra.data$m08))
# 
# sum(table(wra.data$anemia))               #Total women
# 
# 
# #Logistic regression
# options(scipen = 999)
# anemia.model <- glm(anemia ~ mregion + m08, family = "binomial", data = wra.data)
# summary(anemia.model)
# 
# 
# wave1 <- read_dta("C:/Users/Lenovo/Documents/Wave 1/Wave 1/sect5_Access_r1.dta")
# #str(wave1)
# 
# table(wave1$s5q1a1)
# prop.table(table(wave1$s5q1a1))*100
# sum(table(wave1$s5q1a1))
# 
# 
#IHS Data
...
#Remove duplicates, and order by team
summary_by_team <- candidates %>% 
...
#Practice with Malawi data

mlw_eas <- vect("shapefiles/2018_MPHC_EAs_Final.shp")
str(mlw_eas)
crs(mlw_eas)

mlw_eas

as.data.frame(mlw_eas)
dim(as.data.frame(mlw_eas))


#Ploting

plot(mlw_eas)
plot(mlw_eas, "DIST_NAME")


#Plotting with ggplot2
###########################################################

mlw_districts <- vect("shapefiles/Malawi_Districts_2018.shp")
as.data.frame(mlw_districts)

plot(mlw_districts)
plot(mlw_districts, "DIST_NAME")
...
mlw_districts <- st_read("shapefiles/Malawi_Districts_2018.shp")
ggplot() + geom_sf(data = mlw_districts)
...
max(table(mpg$manufacturer))
sort(table(mpg$manufacturer), decreasing = TRUE)[1]   #Mode of models


table(mpg$manufacturer, mpg$year)
cbind(table(mpg$manufacturer, mpg$year))
rowSums(table(mpg$manufacturer, mpg$year))

#Cross tabulation together with row totals
cbind(table(mpg$manufacturer, mpg$year), total = rowSums(table(mpg$manufacturer, mpg$year)))

cbind(table(mpg$manufacturer, mpg$year)/rowSums(table(mpg$manufacturer, mpg$year)))
...
mpg$cty_k = (1.609344/3.78541178)*mpg$cty
mpg$hwy_k = (1.609344/3.78541178)*mpg$hwy

mpg$cty_es <- 100/mpg$cty_k
mpg$hwy_es <- 100/mpg$hwy_k
...
ggplot(mpg, aes(x = displ, y= hwy, color = class, shape = drv)) + geom_point()
...
ggplot(mpg, aes(displ, hwy)) + geom_point(aes(colour = "blue"))  #Compare with
...
lfs_listing <- read_dta("data/lfs2023_hh_listed.dta")

head(lfs_listing)

dim(lfs_listing)
table(lfs_listing$close_ea)

View(lfs_listing)
sum(lfs_listing$plcae_holder_name=="")
sum(lfs_listing$commn_hh_name=="")

sum(lfs_listing$plcae_holder_name=="" & lfs_listing$commn_hh_name=="")
